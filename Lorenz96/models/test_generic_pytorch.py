...
class MyDataset(TorchDataset):
    def __init__(self, *arrays, do_transform=True):
        self.arrays = arrays
...
torch_dataset=MyDataset(torch_data_input,torch_data_output)
...
torch_dataset_val=MyDataset(torch_data_val_input,torch_data_val_output)
...
if ckpt_list:
    ckpt_list.sort()
    ckpt = torch.load(ckpt_list[-1], weights_only=True)
