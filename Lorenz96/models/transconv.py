...
from torch_geometric.nn import GCNConv, GraphConv, TransformerConv
...
class nnmodel(nn.Module):
  def __init__(self):
    ...
    self.predictor = TransformerConv(self.hidden_feature, self.hidden_feature) ### PREDICTOR
  ...
  #  y_data=[Data(x=y[i], edge_index=self.edge_index, edge_weight=self.edge_weight) for i in range(nbatch)]
    y_data=[Data(x=y[i], edge_index=self.edge_index) for i in range(nbatch)]
    y_batch=Batch.from_data_list(y_data)
#    print(nbatch)
#    print(y_batch.view)
#    print(y_batch.batch)
##    quit()
    y = self.predictor(y_batch.x,y_batch.edge_index).view(nbatch,self.hidden_dim)
#    print(y.shape)
#    quit()
    y = self.decoder(y)
    return y
