...
class nnmodel(nn.Module):
  def __init__(self):
    ...
    self.layers = nn.Sequential(
            nn.RNN(data_input_ext.shape[-1],40),
    )
  def forward(self, x):
    y = self.layers(x)
    return y
...
loss=loss_L2(model(torch_data_input)[0],torch_data_output)
...
for j in range(100,200):
  data_predict=model(torch_data_input[j].unsqueeze(0))[0].detach().numpy() 
